public class Percolation {
	...
	private void init() {
		for (int j = 1; j <= N; j++) {
			...
			backwash.union(getPos(1, j), virtualTop);
	...
	private void unionNeighbors(int i, int j) {
		if (isValid(i + 1, j) && isOpen(i + 1, j)) {
			...
			backwash.union(getPos(i, j), getPos(i + 1, j));
		}
		if (isValid(i - 1, j) && isOpen(i - 1, j)) {
			sets.union(getPos(i, j), getPos(i - 1, j));
			backwash.union(getPos(i, j), getPos(i - 1, j));
		...
		if (isValid(i, j + 1) && isOpen(i, j + 1)) {
			...
			backwash.union(getPos(i, j), getPos(i, j + 1));
		}
		if (isValid(i, j - 1) && isOpen(i, j - 1)) {
			sets.union(getPos(i, j), getPos(i, j - 1));
			backwash.union(getPos(i, j), getPos(i, j - 1));
	...
	public boolean isFull(int i, int j) {
		...
		return isOpen(i, j) && backwash.connected(getPos(i, j), virtualTop);
	}
